...
class FindWordTest : CobolTest() {

    override fun defaults(spec: RecipeSpec) {
        spec.recipe(FindWord("CM102M", true))
    }
    ...
    @Test
    fun wordIsNotUsed() = rewriteRun(
        cobolCopy(getNistSource("DB101A.CBL"))
    )

    @Test
    fun cm102mExactMatch() = rewriteRun(
        cobolCopy(
            getNistSource("CM102M.CBL"),
            cm102mExactMatch
        ) { spec ->
            spec.afterRecipe { cu ->
    ...
    @Test
    fun cm102mPartialMatch() = rewriteRun(
        { spec -> spec.recipe(FindWord("cm.*", false))},
        cobolCopy(
            getNistSource("CM102M.CBL"),
            cm102mPartialMatch
        ) { spec ->
