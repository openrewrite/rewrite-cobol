...
public class CobolPreprocessorReplaceTest extends CobolTest {
    ...
    void sm201A() {
        rewriteRun(cobolPreprocessorCopy(getNistResource("SM201A.CBL"), spec -> {
            spec.afterRecipe(cu -> {
                ...
                String result = outputCapture.getOut();
                assertThat(StringUtils.trimIndentPreserveCRLF(result)).isEqualTo(
                  """
                    LABEL RECORDS STANDARD                                      \s
                    VALUE OF                                                    \s
                    XXXXX074                                                    \s
                    IS                                                          \s
                    XXXXX075                                                    \s
                ...
                result = outputCapture.getOut();
                assertThat(StringUtils.trimIndentPreserveCRLF(result)).isEqualTo(
                  """
                            .                                                   \s
                    02 TF-1 PICTURE 9(5).                                  \s
...
//    )
//
//    /* ReplaceArea REPLACE tests */
//    @Test
//    fun sm208A() = rewriteRun(
//      cobolPreprocessorCopy(getNistSource("SM208A.CBL")) { spec ->
//      spec.afterRecipe { cu ->
//      val outputCapture = PrintOutputCapture<ExecutionContext>(InMemoryExecutionContext())
//        val postProcessPrinter = CobolPreprocessorPrinter<ExecutionContext>(false, true)
